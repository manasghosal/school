...
public class TaskFragmentParent extends Fragment {

	private OnItemClickListener mListener;
	private ArrayList<TaskParent> taskParentArrayList;
	private RecyclerView recyclerView;

	public static TaskFragmentParent newInstance(){
		TaskFragmentParent mFragment = new TaskFragmentParent();
		return mFragment;
	}
	...
	public void getDataFromJSON(){
		taskParentArrayList = new ArrayList<>();
		String data = GlobalPreferenceManager.getParentTasks();
		LogUtils.i(data);
		try {
			...
			for (int i=0; i<array.length(); i++){
				...
				String assignmentDate = object.getString("AssignmentDate");
				String assignmentDueDate = object.getString("AssignmentDueDate");
				String assignmentTitle = object.getString("AssignmentTxt");
				String assignmentDescription = object.getString("Remark");

				TaskParent taskParent = new TaskParent(assignmentId, assignmentDate,assignmentDueDate, assignmentTitle,assignmentDescription);
				taskParentArrayList.add(taskParent);
			}
	...
	private void initListView(View rootView){
		...
		TaskParentAdapter adapter = new TaskParentAdapter(getActivity(), taskParentArrayList);
...
//		});
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == Constant.PARENT_TASK_DETAILS_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
			String posVal = data.getStringExtra("POSITION");
			LogUtils.i(" i m here " + posVal);

			if(posVal.isEmpty() || posVal.equals(null))
				;
			else {
				int pos = Integer.valueOf(posVal);
				taskParentArrayList.get(pos).setSubmit(true);
			}
			TaskParentAdapter adapter = new TaskParentAdapter(getActivity(), taskParentArrayList);
			recyclerView.setAdapter(adapter);
