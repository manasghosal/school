...
public class DriverLocationAsyncTask extends AsyncTask<Void, Void, Integer>{
	...
	public DriverLocationAsyncTask(Context context, int task, String email, String uniqueID, String latitude, String longitude) {
		mContext = context;
		mTask = task;
		mEmail = email;
	...
	protected Integer doInBackground(Void... params) {
		...
		try {
			...
			jsonLoginInfo.put("Latitude", latitude);
			jsonLoginInfo.put("Longitude", longitude);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Request request = new Request.Builder()
				.url(Constant.SERVER_BASE_ADDRESS+"api/quicksoftuser/locationdriver")
				.post(body)
				.build();
