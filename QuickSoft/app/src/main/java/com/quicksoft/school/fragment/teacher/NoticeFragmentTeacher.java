...
public class NoticeFragmentTeacher extends Fragment implements View.OnClickListener, SyncCompleteCallback {
	private OnItemClickListener mListener;
	private FancyButton btnClass, btnSection, btnDate, btnStudent;
	private FancyButton btnAttachment, btnSendNotice;
	...
	public static NoticeFragmentTeacher newInstance(String text){
		NoticeFragmentTeacher mFragment = new NoticeFragmentTeacher();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View rootView = inflater.inflate(R.layout.fragment_teacher_notice, container, false);
		...
		btnSendNotice = rootView.findViewById(R.id.btnSendNotice);
			...
			public void onClick(View view) {
				...
				if(btnClass.getText().equals("Select Class") || btnSection.getText().equals("Select Section") || btnDate.getText().equals("Select Date")){
					...
					if(NetworkUtils.isConnected()) {
						mSyncManager = new SyncManager(getActivity(), NoticeFragmentTeacher.this);
	...
	public void onSyncComplete(int syncPage, int response, Object data) {
		...
		}else if(syncPage==Constant.SYNC_TEACHER_NOTICE){
			if(response == Constant.SUCCESS) {
				LogUtils.i(((JSONObject) data).toString());
...
//				btnSection.setText("Select Section");
				...
				editTopic.setText("");
				pDialog.dismiss();
				Toasty.info(getActivity(),"Notice submitted", Toast.LENGTH_LONG).show();
			}
		}
    ...
    public void onClick(View view) {
		...
		if(view == btnSendNotice){
			if(btnClass.getText().equals("Select Class") || btnSection.getText().equals("Select Section") || btnDate.getText().equals("Select Date")){
				...
				new FancyBottomSheetDialog.Builder(getActivity())
						.setMessage("Did you forget to add Notice/Heading?")
						.setBackgroundColor(getResources().getColor(R.color.colorPrimary))
						.setIcon(R.drawable.ic_assignment_late_white_24dp,true)
						.setPositiveBtnText("Ok")
						.setNegativeBtnText("")
				...
				mSyncManager.teacherPostNotice(email, uniqueID, classs, section, dueDate, editTopic.getText().toString(), editDesc.getText().toString(), isAllStudent, studentIdList);
			}
