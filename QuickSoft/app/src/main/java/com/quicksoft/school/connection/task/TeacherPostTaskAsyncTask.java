...
public class TeacherPostTaskAsyncTask extends AsyncTask<Void, Void, Integer>{


	private String  mEmail, uniqueID, classs, section, dueDate, topic, desc;
	...
	public TeacherPostTaskAsyncTask(Context context, int task, String email, String uniqueId, String classs, String section, String dueDate, String topic, String desc, boolean isAll, ArrayList<String> studentList) {
		mContext = context;
		mTask = task;
		mEmail = email;
	...
	protected Integer doInBackground(Void... params) {
		...
		try {
			...
			jsonLoginInfo.put("DueDate", dueDate);
			jsonLoginInfo.put("AssignmentTxt", topic);
			jsonLoginInfo.put("TeacherRemark", desc);
			jsonLoginInfo.put("Class", classs);
			jsonLoginInfo.put("Section", section);
			jsonLoginInfo.put("All", isAllStudent);
			jsonLoginInfo.put("StudentIdList", new JSONArray(studentList));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Request request = new Request.Builder()
				.url(Constant.SERVER_BASE_ADDRESS+"api/quicksoftuser/givetasknoticeteacher")
				.post(body)
				.build();
