...
public class TeacherPostStudentAttendanceAsyncTask extends AsyncTask<Void, Void, Integer>{
	...
	public TeacherPostStudentAttendanceAsyncTask(Context context, int task, String email, String uniqueID, JSONArray jsonArray) {
		mContext = context;
		mTask = task;
		mEmail = email;
	...
	protected Integer doInBackground(Void... params) {
		...
		try {
			...
			jsonLoginInfo.put("StudentAttendanceList", jsonArray);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Request request = new Request.Builder()
				.url(Constant.SERVER_BASE_ADDRESS+"api/quicksoftuser/markattendanceteacher")
				.post(body)
				.build();
