...
public class SummeryAttendanceAdapter extends RecyclerView.Adapter<SummeryAttendanceAdapter.ViewHolder> {

    private ArrayList<AttendanceStudent> data;
    private Context mContext;
    AttendanceCallback attendanceCallback;

    public SummeryAttendanceAdapter(Context context, ArrayList<AttendanceStudent> data, AttendanceCallback attendanceCallback) {
        this.data = data;
        mContext = context;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View v = inflater.inflate(R.layout.adapater_summery_attendance, parent, false);
        ...
        holder.btnAttendance.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(text.compareTo("Late")==0){
                    ...
                    holder.btnAttendance.setText("Late");
                    holder.btnAttendance.setBackgroundColor(mContext.getResources().getColor(R.color.paleRed));
                    data.get(pos).setRemark("L");
                }

                int late=0;
                int absent =0;
                for(int i=0; i< data.size(); i++) {
                    if(data.get(i).getRemark().compareTo("L")==0) {
                        late++;
                    }else if(data.get(i).getRemark().compareTo("A")==0) {
                        absent ++;
                    }
                }
                attendanceCallback.onAttandance(late, absent);
            }
        });
    }
