...
public class IndiAttendanceAdapter extends RecyclerView.Adapter<IndiAttendanceAdapter.ViewHolder> {
    ...
    public IndiAttendanceAdapter(Context context, ArrayList<AttendanceStudent> data, AttendanceCallback attendanceCallback) {
        mContext = context;
        this.data = data;
...
//        displayWidth = size.x;
//        displayDensity = ((Activity)mContext).getResources().getDisplayMetrics().density;
//        LogUtils.i("Screen width * height: "+ displayWidth + " X " + displayHeight + " and Density: "+ displayDensity);
    }

    @Override
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        View v = inflater.inflate(R.layout.adapater_indi_attendance, parent, false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(final ViewHolder holder, int position) {
        Glide.with(holder.itemView.getContext()).load(data.get(position).getImageUrl()).apply(new RequestOptions().placeholder(R.drawable.ico_student_individual)).into(holder.image);
