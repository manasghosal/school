...
public class UpdatePasswordAsyncTask extends AsyncTask<Void, Void, Integer>{


	private String  mEmail, uniqueID, oldPass, newPass;
	private int mTask;
	...
	public UpdatePasswordAsyncTask(Context context, int task, String email, String uniqueID, String oldPass, String newPass) {
		mContext = context;
		mTask = task;
		mEmail = email;
	...
	protected Integer doInBackground(Void... params) {
		...
		try {
			...
			jsonLoginInfo.put("OldPassword", oldPass);
			jsonLoginInfo.put("NewPassword", newPass);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Request request = new Request.Builder()
				.url(Constant.SERVER_BASE_ADDRESS+"api/quicksoftuser/changepassword")
				.post(body)
				.build();
